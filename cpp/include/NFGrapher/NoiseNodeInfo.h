...
struct NoiseNodeInfo : public NodeInfo {
  

  NoiseNodeInfo(const std::string &id)
    : NodeInfo(id), _when(0L), _duration(0L) {}

  NoiseNodeInfo(const nfgrapher::Node &sn)
    : NodeInfo(sn.id), _when(0L), _duration(0L) {
    if (sn.kind != kind()) {
      std::string msg = "cannot initialize NoiseNodeInfo from Node kind = ";
      msg += sn.kind;
  ...
  virtual ~NoiseNodeInfo() {}

  long _when;
  long _duration;
  
  static constexpr const char *kind() { return "com.nativeformat.plugin.noise.noise"; }
  static constexpr const char *name() { return "noise"; }
