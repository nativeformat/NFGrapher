...
struct ExpanderNodeInfo : public NodeInfo {
    ...
    if (sn.config) {
      if (sn.config->count("detectionMode")) {
        ...
        if (!detectionModeMap().count(key)) {
          std::string msg = key;
          msg += " is not a valid value for ExpanderNodeInfo::DetectionMode. ";
          msg += "Must be one of: ";
          msg += "max, rms";
          throw std::invalid_argument(msg);
        } else {
      ...
      if (sn.config->count("kneeMode")) {
        ...
        if (!kneeModeMap().count(key)) {
          std::string msg = key;
          msg += " is not a valid value for ExpanderNodeInfo::KneeMode. ";
          msg += "Must be one of: ";
          msg += "hard, soft";
          throw std::invalid_argument(msg);
        } else {
  ...
  static constexpr const char *kind() { return "com.nativeformat.plugin.compressor.expander"; }
  static constexpr const char *name() { return "expander"; }
